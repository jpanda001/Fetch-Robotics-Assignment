...
For Part 1 of this assignment, a warehouse with the layout shown in Figure 1 is created.
...
| Shelf Model Files | [shelf](https://utoronto-my.sharepoint.com/:f:/g/personal/jyoti_panda_mail_utoronto_ca/Elp38D4DWFlIh-Cr4Uq17HEBuQopKcgtKWcuQkEgNpcnwQ?e=k4xA8t) |
| Floor Model Files | [floor](https://utoronto-my.sharepoint.com/:f:/g/personal/jyoti_panda_mail_utoronto_ca/EmCZwVyvJ1xBiUfp_KJdheIBuc-rJWtwbWlX-1__cl6GVg?e=DwMkWq) |
| Environment World File | [warehouse.world](https://utoronto-my.sharepoint.com/:u:/g/personal/jyoti_panda_mail_utoronto_ca/EVLiHg4gWcBMuTUfTrtrPtABRqwCxJkP4uG9xHV13mcqWA?e=KaGavI) |
...
First of all, the Freight robot model is initialized in a empty environment to verify if the robot model is being rendered properly. This is done by running the command `roslaunch fetch_gazebo simulation.launch robot:=freight`. Figure 3 shows the Freight robot in an empty environment. 
...
Next, the robot model is modified to include a 3D Depth Sensor. Gazebo's Depth Camera Plugin is used to create and integrate a 3D Depth Sensor into the robot. This is done by modifying the `freight.gazebo.xacro` and `freight.urdf` files. `freight.gazebo.xacro` is used for integrating the 3D Depth Sensor into the robot at a predefined frame location. In this file, we also define sensor parameters and the topic names. `freight.urdf` is used to define the frame locations for the sensor. Modified `freight.gazebo.xacro` and `freight.urdf` can be found in the links below.

| Robot `urdf` file | [freight.urdf](https://utoronto-my.sharepoint.com/:u:/g/personal/jyoti_panda_mail_utoronto_ca/Eao-mc0iLQBIul7diFs1ZYoBiw76t6dOfgv2gDqVBx8rGA?e=E7vhA3) |
| Robot `xacro` file | [freight.gazebo.xacro](https://utoronto-my.sharepoint.com/:u:/g/personal/jyoti_panda_mail_utoronto_ca/EYMZuPpijbJIjKu-lB3-QkcB_gBPZhLcvqkz9U_JG5ODDQ?e=yHRfHw) |
...
When the Freight robot simulation is started, numerous topics are created and published. Following code snippet shows the list of all topics being published:
```
...
Out of all the topics listed above, following topics are relevant for mapping, planning and testing:
 ...
 - `/base_scan` or `/base_scan_raw`: These topics show what the 2D Lidar of the Freight Robot is currently observing. 2D Lidar Pointcloud/Laserscan frames can be sent into a SLAM algorithm to generate a map of the robot environment and localize or provide a pose of the robot within that robot environment. Robot pose is very useful while planning a path and ensuring that the robot follows the path.
 -  `/camera/color/image_raw`: This topic can be used to obtain images of the robot environment. These images can then be used to conduct object classification to differentiate between static and dynamic obstacles. This information can then be used to produce a better map of the robot environment.
 - `/camera/depth/points`: This topic produces a dense 3D-Pointcloud representation of the robot environment. This can be used to improve the accuracy of SLAM and improve overall quality of the map produced.
 -  `/odom`: This topic provides the current linear and angular velocity of the robot calculated based on wheel rotation speeds. Actual robot velocities can be very useful while implementing path-following control algorithms. A well-functioning control algorithm is required to test if a robot can follow a path to reach a goal position. This topic also provides pose of the robot relative to its starting positon which is calculated using wheel odometry. This can be very useful for a SLAM algorithm. However, due to wheel slippage, wheel odometry is generally very inaccurate and not quite useful. 
 - `/query_controller_states/feedback`, `/query_controller_states/goal`, `/query_controller_states/result` and `/query_controller_states/status`: These topics can be very useful to evaluate the high-level states of a robot during test runs. These topics can be used to check what the start and goal positions for a robot are and generate a path based on that. These topics can be used to check where on a path a robot is. These topics can be used to also check what the current status of a robot is and if the robot has encountered any faults.
...
| Rosbag File | [robot_moving.bag](https://utoronto-my.sharepoint.com/:u:/g/personal/jyoti_panda_mail_utoronto_ca/EVKkQqpBbpdLk9SCPioyMxQBud7n5KdVTRPUt_wPQWhBew?e=yyflfX) |